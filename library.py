...
def transactions(wallet_origin, wallet_destin, params):
    ...
    if balance[0]['lovelace'] < target_calculated:
        ...
        if deplete: 
            ...
            addr.append([addr_destin + '+' + str(int(target)) + '+' + str(tokenamount) + ' ' + str(policyid) + '.' + str(tokenname)])
        else:
        ...
        command_string = [
            ...
            'transaction', 'sign',
            '--tx-body-file', transaction_path_file + '/tx.draft',
            '--signing-key-file', keys_file_path + '/' + wallet_id + '/' + wallet_id + '.payment.skey',
            '--testnet-magic', str(CARDANO_NETWORK_MAGIC),
            '--out-file', transaction_path_file + '/tx.signed']
        
        i = 0
        if mint != []:
            command_string.insert(7+i, '--signing-key-file')
            command_string.insert(8+i, keys_file_path + '/' + wallet_id + '/minting/' + wallet_id + '.policy.skey')
        print(command_string)
        subprocess.check_output(command_string)
